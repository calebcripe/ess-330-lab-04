...
```{r}
giant_sally_clean <- and_vertebrates  |>  
  filter(species == "Coastal giant salamander") |> 
  drop_na(unittype, section)

sally_cont_table <- table(giant_sally_clean$section, giant_sally_clean$unittype)

chisq.test(sally_cont_table)
```

When conducting a chi-square test for the filtered Giant coastal salamander data I found that the p-value was very small (p < 2.2e-16), indicating that there is a significant relationship between forest type (section) and channel unit (unittype) for this species. Additionally, the degrees of freedom value (df = 5) and the x-squared value (x-squared = 200.71) demonstrate that the test ran successfully. 
...
```{r}
giant_sally_filtered <- giant_sally_clean |> drop_na(weight_g)

giant_sally_filtered |> 
  drop_na(weight_g) |> 
  ggpubr::ggviolin(x = "section", 
                   y     = "weight_g", 
                   add   = "boxplot",
...
cc_weight <- giant_sally_filtered |> 
  filter(section == "CC") |> 
  pull(weight_g)

og_weight <- giant_sally_filtered |> 
  filter(section == "OG") |> 
  pull(weight_g)
...
t.test(log(giant_sally_filtered$weight_g) ~ giant_sally_filtered$section, var.equal = TRUE)
```

When testing the hypothesis that there is a significant difference in species biomass between clear cut and old growth forest types, I found that the hypothesis is supported by the data testing I conducted. After comparing the variances of the two variables and normalizing the data with a log-transformation, the mean weight for the Giant coastal salamander was 1.829 for the clear cut forest and 1.576 for the old growth forest, suggesting that there is a distinct difference in weight. This is supported by a very small p-value (p < 2.2e-16) which indicates that the null hypothesis is soundly rejected. 
...
When testing the correlation between body length (snout to fork length) and body mass for Cutthroat trout, I found that there is a very strong correlation between the two variables. Both the Pearson's and Spearman's correlation tests found that there was a correlation coefficient of 0.99 and a very small p-value (p < 2.2e-16), which strongly suggests that there is a definitive correlation between body length and body mass.
...
```{r}
# data set
install.packages("lterdatasampler", repos = "https://cloud.r-project.org/")
library(lterdatasampler)
data("pie_crab")
```
...
```{r}
library(ggplot2)
pie_sites_2 <- pie_sites %>%
  select(site, size, latitude) %>%
  arrange(latitude)

ggplot(pie_sites_2, aes(x = site, y = size, fill = site)) +
  geom_boxplot() +
  scale_x_discrete(limits = unique(pie_sites_2$site)) +
  labs(title = "Carapace Width by Site",
       x = "Site", 
       y = "Carapace Width (cm)") +
  theme_bw()
```

After completing an ANOVA test and post-hoc Tukey's HSD to test for significant differences in crab size among 3 different sites, ordered by latitude, I created a box plot to graph the differences between the 3 sites. There appears to be a statistically significant effect of site latitude on crab size, with a very small p-value (p < 2.2e-16) suggesting that the null hypothesis can be rejected. The size of the crabs increases with latitude, as demonstrated by the box plot, which reflects the results of the statistical tests conducted to measure this effect.
...
After conducting a linear regression for the effect of annual variation of water temperature on carapace width, there appears to be a slight increase in carapace width with increase in annual variation. The p value (p = 0.5275), which is quite large, suggests that there is no significant correlation between the two variables though, indicating that annual variation of water temperature does not have a statistically significant effect on carapace width. 
...
After conduct a multiple linear regression for the effects of latitude, annual variance in air temperature, and annual variance in water temperature on carapace width, the results suggest that there is a statistically significant effect of these variables on carapace width. A very small p-value (p < 2.2e-16) suggests that the null hypothesis can be rejected, and that these variable in combination effect the carapace width of Fiddler crabs. 
